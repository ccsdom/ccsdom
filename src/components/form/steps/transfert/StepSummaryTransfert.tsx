...
import ConfirmationNumberIcon from "@mui/icons-material/ConfirmationNumber"; // Icône pour SIREN/SIRET
import { FormData, SectionEditable } from "../types/form";

interface StepSummaryTransfertProps {
  data: FormData;
  onBack: () => void;
  onNext: () => void;
...
// Map des codes statut juridique vers leur libellé complet
...
const formatStatutJuridique = (code?: string): string => {
  if (!code) return "Non renseigné";
  return formeJuridiqueMap[code] || code;
};
  ...
  >
    <Box display="flex" alignItems="flex-start" gap={2}>
      ...
      <Box>
        ...
        <Typography>{value || "Non renseigné"}</Typography>
      </Box>
    </Box>
    {onEdit && (
      <IconButton onClick={onEdit} size="small" sx={{ color: "#7c3aed" }}>
...
const StepSummaryTransfert: React.FC<StepSummaryTransfertProps> = ({
  data,
  onBack,
  onEdit,
  onNext,
  ...
  const formatAdresseDomiciliation = (): string => {
    if (!data.adresseComplete) return "Non renseignée";
    const prix = data.prixAdresse ? `\n${data.prixAdresse} € / mois (HT)` : "";
    return `${data.adresseComplete}${prix}`;
  };

  const formatOffreCourrier = (): string => {
    const label = data.libelleOffreCourrier ?? "Offre non sélectionnée";
    const prix = data.prixOffreCourrier ?? "—";
    return `${label}\n${prix}`;
  };
  ...
  return (
    ...
    >
      <Typography variant="h4" fontWeight="bold" mb={1}>
        Récapitulatif de votre transfert
      </Typography>
      <Typography variant="subtitle1" color="text.secondary" mb={4}>
        Vérifiez que toutes les informations ci-dessous sont correctes avant de finaliser le transfert.
      </Typography>

      {/* Projet */}
      <InfoBlock
        label="Projet choisi"
        value={data.projet === "transfert" ? "Transfert de siège" : "Non renseigné"}
        icon={<DomainIcon color="primary" />}
        onEdit={() => onEdit("projet")}
      />

      {/* Informations de l'entreprise récupérées via INSEE */}
      <InfoBlock
        label="Dénomination sociale"
        value={data.nomEntreprise ?? "Non renseignée"}
        icon={<DomainIcon color="primary" />}
      ...
      <InfoBlock
        label="Numéro SIRET"
        value={data.siret ?? "Non renseigné"}
      ...
      <InfoBlock
        ...
        value={formatStatutJuridique(data.statutJuridique)}
      ...
      <InfoBlock
        ...
        value={
          data.optionTransfert === "self"
            ? "Je gère le transfert seul·e"
            : data.optionTransfert === "expert"
      ...
      <Stack direction="row" justifyContent="space-between" mt={4}>
        ...
        <Button
          ...
          title={
            isNextDisabled
              ? "Veuillez compléter le statut juridique et la gestion du courrier"
              : undefined
          }
        >
          Valider
...
export default StepSummaryTransfert;
